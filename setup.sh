...
Step 2: Conda environment called vir2 will be automatically created and provisioned.

"

conda --version > /dev/null 2>&1 || { echo >&2 "Virominer2 setup requires conda. Aborting."; exit 1; }
...
conda env create --file $CONTAINING_DIRECTORY/environment.yml || 
conda env update --file $CONTAINING_DIRECTORY/environment.yml

echo "Conda env created.  Setting up paths..."
source activate vir2
[[ ! -d $CONDA_ENV_PATH ]] && { echo "CONDA_ENV_PATH not a dir!"; source deactivate; exit; }
#PTH_FILE=$CONDA_ENV_PATH/lib/python3.5/site-packages/vir2.pth
...
echo "Setup complete.  Type source activate vir2 to activate environment."
